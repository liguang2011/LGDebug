...
#define SIZE_fpsLabel CGSizeMake(50, 15)
#define FONT_SIZE_fpsLabel (13)
...
- (id)init {
    if (self = [super init]) {
        ...
        self.fpsLabelPosition = FPSIndicatorPositionTopRight;
...
//    _fpsLabel.font = [UIFont boldSystemFontOfSize:FONT_SIZE_fpsLabel];
    ...
    _fpsLabel.alpha = 0.9;
    _fpsLabel.layer.cornerRadius = 4.0f;
...
- (void)displayLinkTick:(CADisplayLink *)link {
    ...
    NSMutableAttributedString *fpsStr = [[NSMutableAttributedString alloc] initWithString:text];
    [fpsStr addAttributes:@{NSFontAttributeName : [UIFont boldSystemFontOfSize:FONT_SIZE_fpsLabel],
                            NSForegroundColorAttributeName : [UIColor orangeColor]}
                    range:NSMakeRange(0, text.length)];
    [fpsStr addAttributes:@{NSFontAttributeName : [UIFont boldSystemFontOfSize:FONT_SIZE_fpsLabel],
                           NSForegroundColorAttributeName : [UIColor whiteColor]}
                    range:NSMakeRange(text.length - 3, 3)];
    [_fpsLabel setAttributedText:fpsStr];
}
...
- (void)setFpsLabelPosition:(FPSIndicatorPosition)fpsLabelPosition {
    ...
    switch (_fpsLabelPosition) {
        case FPSIndicatorPositionTopLeft:
            _fpsLabel.frame = CGRectMake(Is_iPhoneXSerious?25:(kScreenWidth - SIZE_fpsLabel.width) / 2 - PADDING_LEFT_fpsLabel - 1, 2.5, SIZE_fpsLabel.width, SIZE_fpsLabel.height);
            break;
        case FPSIndicatorPositionTopRight:
            _fpsLabel.frame = CGRectMake(Is_iPhoneXSerious?kScreenWidth - 60:(kScreenWidth + SIZE_fpsLabel.width) / 2 + (PADDING_RIGHT_fpsLabel) , 2.5, SIZE_fpsLabel.width, SIZE_fpsLabel.height);
            break;
        case FPSIndicatorPositionBottomCenter:
            _fpsLabel.frame = CGRectMake(Is_iPhoneXSerious?25:(kScreenWidth - SIZE_fpsLabel.width) / 2 + PADDING_CENTER_fpsLabel, PADDING_TOP_fpsLabel, SIZE_fpsLabel.width, SIZE_fpsLabel.height);
            break;
        default:
            break;
    }
